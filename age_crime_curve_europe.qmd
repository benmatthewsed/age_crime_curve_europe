...
-   We can analyse change in the demographics of crime to refine theories of the crime drop, some explanations for the crime drop fit better with period effects (implying a reasonably uniform change across age?) (Kim et al. 2016)
...
![](resources/heuristic_data_access.png)

## Research Design

![](resources/heuristic_data_access_2.png)
...
-   No data (that I could find) for Sweden or the Netherlands! Though through some non-systematic Googling, there was data available for Switzerland, New Zealand and South Korea...
...
-   All sanctions?
    -   In Norway I removed on the spot fines because that's [what SSB do](https://www.ssb.no/en/sosiale-forhold-og-kriminalitet/kriminalitet-og-rettsvesen/statistikk/straffereaksjoner) (and the age-crime curve looked very odd if I didn't) otherwise yes

## Methods

-   Convert aggregated counts of convictions and population estimates into single-year estimates of conviction rates

-   Used `R` implementation of Penalized Composite Link Model (PCLM) using the package [`ungroup`](https://cran.r-project.org/web/packages/ungroup/index.html)

-   PCLM models convictions data and population data together to estimate a smooth surface of conviction rates
...
-   Focusing on age and year not age and cohort
...
-   In the future - possibly bespoke models and visualizations for comparing Lexis surfaces (Acosta & van Raalte, 2019)?
...
-   And the timings of biggest falls in youth convictions (cohorts born around 1990ish) are pretty consistent across the four countries (roughly after 2005?)
...
-   So maybe perhaps this does fit better with a youth-focused explanation for the crime drop?
-   Scotland seems like the odd-one-out here?

## Discussion

-   If you believe in the invariance these about the age-crime curve, you almost lose by winning - I'd say that three of the four countries showed pretty 'classic' annual age-crime curves throughout the period analysed, but one didn't
...
-   There are two directions this research could go in:
...
-   Having done this analysis for (some of) Northern Europe, I think maybe an even more maximalist approach would be preferable - extending this comparison to anywhere that publishes data on age and conviction?

## Thank you! {background-image="resources/background.png"}
